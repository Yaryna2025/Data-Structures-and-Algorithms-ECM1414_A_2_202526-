...
# 2. Brute-force Algorithm (multiple constraints)
def brute_force(activities, max_time, max_budget):
    ...
    for s in range(n + 1):
        # Generate all possible combinations of indices of length s.
        for subset in itertools.combinations(range(n), s):
            ...
            if total_time <= max_time and total_cost <= max_budget:
                if total_enjoyment > max_enjoyment:
                    ...
                    best_activities = selected_activities
...
def dynamic_programming(activities, max_time, max_budget):
    ...
    for i in range(n + 1):
        ...
        for t in range(max_time + 1):
            budget_layer = [0] * (max_budget + 1)
            time_layer.append(budget_layer)
        max_enjoyment_table.append(time_layer)

    for i in range(1, n+1):
        activity = activities[i-1]
        for t in range(max_time+1):
            for c in range(max_budget + 1):
                # Check if our activity is suitable for left time and cost.
    ...
    for i in range(n, 0, -1):
        ...
        if max_enjoyment_table[i][t][c] != max_enjoyment_table[i-1][t][c]:
            ...
            # Reduce remaining time and cost by this activity's time and cost.
            t -= activity['time']
            c -= activity['cost']
